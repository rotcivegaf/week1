...
template sudoku() {
    ...
    for (var  i= 0; i < 9; i++) {
        for (var j = 0; j < 9; j++) {
            rangeProofsPuzzle[i][j] = RangeProof(4);
            // assert(puzzle[i][j]>=0);
            // assert(puzzle[i][j]<=9);
            rangeProofsPuzzle[i][j].in <== puzzle[i][j];
            rangeProofsPuzzle[i][j].range[0] <== 0;
            rangeProofsPuzzle[i][j].range[1] <== 9;

            rangeProofsSolution[i][j] = RangeProof(4);
